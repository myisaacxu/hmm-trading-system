...
class TestLogger:
    ...
    def test_log_trading_signal(self):
        ...
        signal = TradingSignal(
            date=pd.Timestamp(datetime.now()), regime="Bear", position=-1, price=100.5
        )
    ...
    def test_log_format(self):
        ...
        logger = Logger(
            log_file=temp_file, log_level=LogLevel.INFO, console_log_level=LogLevel.OFF
        )
    ...
    def test_multiple_loggers(self):
        ...
        with open("/tmp/test_log1.log", "r") as f1, open(
            "/tmp/test_log2.log", "r"
        ) as f2:
            logs1 = f1.read()
            logs2 = f2.read()
    ...
    def test_log_rotation(self):
        ...
        timestamp = pd.Timestamp(datetime.now())
        for i in range(100):
            logger.log_market_state(
                timestamp, f"State_{i % 3}", confidence=0.7 + i % 30 / 100
            )
        ...
        # 验证至少生成了一个备份文件
        backup_files = [
            f for f in os.listdir("/tmp") if f.startswith("test_hmm_log.log.")
        ]
